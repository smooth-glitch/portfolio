...
type RippleGridProps = {
  ...
  gridColor?: string;            // hex, e.g. "#ffffff"
  rippleIntensity?: number;      // 0.0..?
  gridSize?: number;             // e.g. 10
  gridThickness?: number;        // e.g. 15
  fadeDistance?: number;         // e.g. 1.5
  vignetteStrength?: number;     // e.g. 2.0
  glowIntensity?: number;        // e.g. 0.1
  opacity?: number;              // 0..1
...
export default function RippleGrid({
  ...
  const uniformsRef = useRef<any>(null); // uniforms object from OGL

  useEffect(() => {
    // Guard SSR / missing container
...
void main() {
  ...
  gl_FragColor = vec4(color * t * finalFade * opacity, alpha);
    ...
    return () => {
      ...
      // release WebGL context (helps with HMR in dev)
